...
const ProjectCard = ({direction, img, title, ghLink, liveLink, info, blog, techStack}) => {
    // Use predefined classes for Tailwind to work properly
    ...
    return (
        <div className={`project h-full bg-[#31343D] rounded-xl flex ${directionClasses[direction]} gap-8`}>
            <div className={direction === 'col' ? 'w-full flex-shrink-0' : 'flex-shrink-0'}>
                <img
                    src={img}
                    alt=""
            ...
            <div className={`project__desc flex flex-col p-6 gap-2 ${direction === 'col' ? 'flex-1' : ''}`}>
                <div className="project__heading flex justify-between items-center gap-8">
                    ...
                    <div className="project__links flex gap-2 items-center">
                        { ghLink ?
                            <a href={ghLink}>
                                <svg className="gh" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 24 24">
                        ...
                        { liveLink ?
                            <a href={liveLink}>
                                <svg className="live-link" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     strokeWidth="1" stroke="currentColor" width="24" height="24">
                ...
                <div className="project__info flex-1">
                    <p>{info}</p>
                </div>
                <div className="blog-button my-2">
                    {blog ? <button>{blog}</button> : null}
                </div>
                <div className="tech-stack flex gap-2">
                    {techStack.map((tech, index) => (
                        <a key={index} className="tech cursor-pointer">{tech}</a>
                    ))}
                </div>
            </div>
        </div>
